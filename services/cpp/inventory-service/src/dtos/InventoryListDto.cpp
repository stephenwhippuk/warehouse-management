...
InventoryListDto::InventoryListDto(
    const std::vector<InventoryItemDto>& items,
    int totalCount,
    int page,
    int pageSize,
    int totalPages)
...
void InventoryListDto::validateNonNegativeInteger(int value, const std::string& fieldName) const {
    if (value < 0) {
        throw std::invalid_argument(fieldName + " must be non-negative");
    }
}

void InventoryListDto::validatePositiveInteger(int value, const std::string& fieldName) const {
    if (value < 1) {
        throw std::invalid_argument(fieldName + " must be positive (greater than 0)");
    }
}

json InventoryListDto::toJson() const {
