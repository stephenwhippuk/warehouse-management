...
WarehouseListDto::WarehouseListDto(
    const std::vector<WarehouseDto>& items,
    int total,
    int page,
    int pageSize)
    : items_(items)
...
void WarehouseListDto::validateNonNegativeInteger(int value, const std::string& fieldName) const {
    if (value < 0) {
        throw std::invalid_argument(fieldName + " must be non-negative");
    }
}

void WarehouseListDto::validatePositiveInteger(int value, const std::string& fieldName) const {
    if (value < 1) {
        throw std::invalid_argument(fieldName + " must be positive (>= 1)");
    }
}

json WarehouseListDto::toJson() const {
