#include "product/utils/RabbitMqMessageBus.hpp"
...
RabbitMqMessageBus::RabbitMqMessageBus(const MessageBus::Config& config)
    ...
    try {
        ...
        if (auto logger = utils::Logger::getLogger()) {
            logger->info("Connected to RabbitMQ at {}:{} vhost={} exchange={}",
                        config_.host, config_.port, config_.virtual_host, config_.exchange);
        }
    } catch (const std::exception& ex) {
        if (auto logger = utils::Logger::getLogger()) {
            logger->error("Failed to initialize RabbitMQ message bus: {}", ex.what());
        }
...
void RabbitMqMessageBus::publish(const std::string& routingKey,
    ...
    if (!connection_) {
        ...
        if (auto logger = utils::Logger::getLogger()) {
            logger->warn("RabbitMQ message bus not connected; skipping publish for routing key {}", routingKey);
        }
        return;
    }
    ...
    if (status != AMQP_STATUS_OK) {
        if (auto logger = utils::Logger::getLogger()) {
            logger->error("Failed to publish message to RabbitMQ (routing key {}): {}",
    ...
    if (auto logger = utils::Logger::getLogger()) {
        logger->debug("Published message to RabbitMQ exchange={} routingKey={} payloadSize={} bytes",
